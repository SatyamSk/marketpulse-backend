...
def fetch_news() -> list[dict]:
    ...
    for feed_url, feed_label in RSS_FEEDS:
        try:
            ...
            print(f"    OK     {feed_label:38} → {count}")
            ok += 1

        except Exception as e:
            print(f"    FAIL   {feed_label:38} → {e}")
            fail += 1

    print(f"\n  Sources: {ok} OK · {fail} failed · {len(headlines)} unique headlines")
    return headlines
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Find NON-OBVIOUS opportunities — second and third order beneficiaries. "
                        "Example: highway contract benefits contractor AND cement, steel, logistics. "
                        "Geopolitics is NOT a sector — use actual affected market sector. "
                        f"{govt_note}"
                ...
                {
                    "role": "user",
                    "content": f"""Analyze this headline and return JSON with exactly these keys:
...
- arousal: float 0.0-1.0 (0=calm, 1=alarming)
...
- contrarian_flag: boolean (true if real implication is opposite of surface)
- contrarian_reason: string, explain if contrarian_flag true else empty string
...
Description: {headline["description"][:400]}""",
        ...
        return {
            ...
            "macro_sensitivity": "medium", "one_line_insight": "",
            "signal_reason": "", "contrarian_flag": False, "contrarian_reason": "",
        }
...
def run_pipeline():
    ...
    for i, h in enumerate(headlines):
        ...
        geo  = "🌍" if labels["geopolitical_risk"] else "  "
        govt = "🏛" if h.get("is_govt_source") else "  "
        print(
            f"    [{i+1:3d}/{len(headlines)}] {geo}{govt} "
            f"[{labels['sector']:12}] [{labels['sentiment']:8}] "
            f"{h['title'][:45]}..."
        )
    ...
    print(f"\n{'─'*60}")
    print(f"  Headlines : {len(headlines_df)}  |  Geo: {geo}  |  Govt: {govt}")
    print(f"  Shocks    : {major} major · {shocks} shock")
    print(f"  Avg CSI   : {avg_csi:+.1f}  |  Avg Risk: {avg_risk:.1f}")
    print(f"  MSI       : {msi['msi']} ({msi['level']})")
    print(f"  Regime    : {regime}  |  Time: {elapsed}s")
    print(f"\n  SECTOR SNAPSHOT:")
